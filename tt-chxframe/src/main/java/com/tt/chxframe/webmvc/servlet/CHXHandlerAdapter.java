...
/**
 * 原生 SpringHandlerAdapte 主要完成请求传递 服务端的参数列表与 Met 实参列表的对
 * 应关系，完成参数值的类型转换工作 方法是 handle(），在 handle(）方法中用反射来调用被适
 * 目标方法，并将转换 装好的参数列表传递过去。
 * @author: guoyitao
 * @date: 2020/11/28 21:30
 * @version: 1.0
 */
public class CHXHandlerAdapter {
    public ModelAndView handle(HttpServletRequest req, HttpServletResponse resp, CHXHandlerMapping handler) throws InvocationTargetException, IllegalAccessException {
        CHXHandlerMapping handlerMapping = (CHXHandlerMapping)handler;
    ...
    public boolean supports(Object handler) {
        return (handler instanceof CHXHandlerMapping);
    }
}
